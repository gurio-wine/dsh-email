...
/** One in-memory device-code login, per account. Never persisted. */
...
const starting = new Map<string, Promise<DeviceFlowStart>>()
 ...
 */
const AADSTS_MESSAGES: Record<string, string> = {
...
export function mapAadstsMessage(code: string | number): string | undefined {
...
function aadstsOf(doc: Record<string, unknown>, description: string): string {
  const codes = Array.isArray(doc.error_codes) ? doc.error_codes : []
  for (const candidate of codes) {
    if (typeof candidate === 'number' || (typeof candidate === 'string' && candidate !== '')) return String(candidate)
  }
 ...
 */
export function classifyOAuthFailure(payload: unknown, httpStatus = 0): OAuthFailure {
  ...
  const base: OAuthFailure = { pending: false, slowDown: false, clearToken: false, message: '', code }
  if (code === 'authorization_pending') return { ...base, pending: true }
  if (code === 'slow_down') return { ...base, pending: true, slowDown: true }
  if (code === 'expired_token') return { ...base, message: '设备码超时，请重新发起登录' }
  if (code === 'authorization_declined') return { ...base, message: '你在登录页拒绝了授权，请重新发起登录' }
  if (code === 'bad_verification_code') return { ...base, message: '设备码无效或已被使用，请重新发起登录' }
  ...
  const mapped = mapAadstsMessage(aadsts)
  if (mapped !== undefined) return { ...base, message: mapped + '（AADSTS' + aadsts + '）' }
...
export function readTokenStore(): OAuth2TokenStore {
  ...
  const accounts: Record<string, OAuth2TokenEntry> = {}
...
function tokenEntryOf(value: unknown): OAuth2TokenEntry | undefined {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) return undefined
  const doc = value as Record<string, unknown>
  if (typeof doc.refreshToken !== 'string' || doc.refreshToken === '') return undefined
  return {
...
export function clearTokenFor(name: string): boolean {
  const store = readTokenStore()
  if (!Object.prototype.hasOwnProperty.call(store.accounts, name)) return false
...
export function oauth2StateOf(name: string, configuredUser?: string, configuredClientId?: string): { state: OAuth2State; user?: string } {
  ...
  if (flow !== undefined) {
    if (Date.now() < flow.expiresAt) return { state: 'pending' }
    pending.delete(name)
  }
  return { state: 'none' }
}

export function isDeviceFlowPending(name: string): boolean {
  return pending.get(name) !== undefined
}

export function clientIdOf(cfg: OAuth2AccountConfig): string {
...
function expiresInMs(payload: Record<string, unknown>): number {
...
function storeToken(name: string, cfg: OAuth2AccountConfig, payload: Record<string, unknown>, fallbackRefresh: string): string {
 ...
 */
export async function startDeviceFlow(name: string, cfg: OAuth2AccountConfig): Promise<DeviceFlowStart> {
  ...
  const task = (async (): Promise<DeviceFlowStart> => {
    ...
    if (url === '') throw new OAuth2Error('微软未返回登录网址，请稍后重试')
...
export type OAuth2PollResult = { status: 'ok'; user: string } | { status: 'pending' }
 ...
 */
export async function pollDeviceFlow(name: string): Promise<OAuth2PollResult> {
  const flow = pending.get(name)
  if (flow === undefined) throw new OAuth2Error('尚未发起设备码登录：请先在设置页点击「登录」')
  ...
  if (verdict.pending) {
    if (verdict.slowDown) flow.interval += 5
...
function refreshAccessToken(name: string, cfg: OAuth2AccountConfig, entry: OAuth2TokenEntry): Promise<string> {
  ...
  const task = (async (): Promise<string> => {
    ...
    if (verdict.clearToken) {
      try { clearTokenFor(name) } catch { /* the message below is what matters */ }
    }
    throw new OAuth2Error(verdict.message)
  })()
  const tracked = task.then(
    token => { refreshing.delete(name); return token },
    error => { refreshing.delete(name); throw error },
  )
